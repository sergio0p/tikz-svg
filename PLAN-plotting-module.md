...
This section captures knowledge from the planning session that is not evident from the plan itself.

### Codebase orientation

- **Working directory:** `/Users/sergiop/Dropbox/Scripts/tikz-svg`
- **All development happens in `src-v2/`** — `src/` is live via symlink at `LECWeb/510/tikz-svg` and must NEVER be edited directly.
- **Tests:** `node --test` from project root. Currently 191 tests, all passing. Tests use `node:test` with `describe`/`it`/`before` and `node:assert/strict`. Integration tests use jsdom for DOM.
- **Package type:** `"type": "module"` in `package.json` — all files are ES modules.
- **No bundler.** All imports are bare ES module specifiers. Browser files use `<script type="module">`.

### math.js specifics

- **Already installed:** `npm install mathjs` was run. Version 15.1.1.
- **Verified import:** `import { compile } from 'mathjs'` works. The `compile()` function returns an object with `.evaluate(scope)` method.
- **Verified evaluation:** `compile('sin(x)').evaluate({ x: Math.PI/2 })` returns `1`. The `pi` and `e` constants are built-in.
- **Performance note:** `compile()` should be called once and the compiled expression reused. The `compileFn()` wrapper in the plan does this correctly.
- **Security:** math.js uses its own expression parser — no `eval()` or `new Function()`. Safe for user-provided expressions.

### PGF source files to consult

All are in `References/`:

| File | What to look at |
|---|---|
| `pgflibraryplothandlers.code.tex` lines 24–105 | The smooth curve (curveto) algorithm. The tension formula is `factor = 0.2775 * tension`. Control points for point P[i] are computed from the chord P[i+1] - P[i-1]. |
| `pgflibraryplothandlers.code.tex` lines 610–700 | The staircase handlers (const plot, const plot mark right, const plot mark mid). Read the ASCII art diagrams in the comments — they explain the geometry precisely. |
| `pgflibraryplothandlers.code.tex` lines 700–810 | The jump handlers. Same as staircase but with moveTo instead of lineTo for vertical steps. |
| `pgflibraryplothandlers.code.tex` lines 280–530 | Comb and bar handlers. Note: bars use `\pgfpathrectangle` which is the PGF path-level rectangle (not a node). |
| `pgflibraryplotmarks.code.tex` | All 27 mark definitions. Each mark is a sequence of `\pgfpathmoveto`/`\pgfpathlineto`/`\pgfpathellipse` commands centered at origin. |
| `pgfmoduleplot.code.tex` lines 1–270 | The plot stream architecture: `\pgfplotstreamstart`, `\pgfplotstreampoint`, `\pgfplotstreamend`. Our `splitAtJumps()` helper is the JS equivalent of PGF's jump handling. |
| `tikz.code.tex` lines 1225–1330 | TikZ-level option defaults: `domain=-5:5`, `samples=25`, `variable=\x`, `tension=0.5`. These values are used as defaults in our evaluator. |

### The smooth curve algorithm in detail

The PGF smooth handler (`\pgfplothandlercurveto`) works as follows:

1. The first point is a moveTo (or lineTo if `\pgfsetlinetofirstplotpoint`).
2. The second point is stored, awaiting the third.
3. From the third point onward, for each point P[i], compute the chord vector V = P[i+1] - P[i-1].
4. Control point before P[i] = P[i] - factor * V.
5. Control point after P[i] = P[i] + factor * V.
6. Emit a cubic Bezier from P[i-1] to P[i] using the previous "after" support and the current "before" support.
7. The last segment uses the last point as its own end support (duplicated).
8. `factor = 0.2775 * tension`. Default tension = 0.5, so default factor = 0.13875.

The closed curve variant (`\pgfplothandlerclosedcurve`) wraps around: P[-1] is the last point, P[n] is the first point. This makes the curve seamless at the join.

### Reviewer findings (already fixed in plan)

- **`star` mark** was implemented as a filled 10-pointed polygon. PGF's `star` is 5 stroked spokes from center. Fixed.
- **`triangle` mark** used ad-hoc vertex positions. Changed to polar vertices at 90°/210°/330° matching PGF.
- **Fill mode distinction** (stroked `o` vs filled `*`) was missing. Added `getMarkFillMode()` and `FILL_MODES` map.
- **2-point curveto** falls back to lineTo (PGF does a curveTo with coincident control points). Acceptable simplification — produces the same visual result.

### Things to watch for during implementation

1. **The `Path` class's `rect()` method** constructs a closed rectangle via moveTo + 3 lineTo + close. The ybar handler uses this. Make sure the rect coordinates are correct — `rect(x, y, w, h)` starts at `(x, y)` and extends by `(w, h)`.

2. **SVG y-down coordinate system.** The evaluator samples in math coordinates (y-up) but the Path class works in SVG coordinates (y-down). The plan does NOT negate y anywhere — this is intentional. The plot module returns raw math coordinates. The caller (or a future emitter integration) is responsible for coordinate system conversion if needed. Document this in the JSDoc.

3. **math.js `compile` vs `evaluate`.** Use `compile()` for repeated evaluation (pre-parses the expression tree). Do NOT use `math.evaluate(expr, scope)` directly in a loop — it re-parses every call.

4. **The `splitAtJumps` helper** is shared across all handlers. If you find a handler that needs different jump behavior (e.g., const plot mark mid needs the previous point's y-value even after a jump), you may need to adjust the helper or handle jumps within the handler itself.

5. **Testing edge cases the reviewer flagged:** Add tests for empty point arrays `[]` and all-undefined point arrays. The handlers should return an empty Path, not crash.

6. **The `closedcurve` handler** has a simplified wrap-around compared to PGF. PGF stores the first few points, processes all interior points, then handles the wrap-around with special first/last segment logic. Our simplified version computes all supports in one pass using modular indexing. This produces correct results for convex shapes but may differ slightly from PGF on highly non-convex point sets. This is acceptable.

### Related work done in this session (before this plan)

1. **TikZ Design Principles Audit** — 9 reports + summary in `audit/`. Covers all aspects of the library against TikZ Chapter 11.

2. **Named styles, groups, pipeline transforms** — implemented and committed (`8c0111e`). Three new features: `config.styles`, `config.groups`, `config.transform`. Style cascade: `DEFAULTS → stateStyle/edgeStyle → group → named style + per-element`. 23 new tests. All 191 tests pass.

3. **E510 LECWeb fix** — `tikz-svg/src/` was copied to the E510 GitHub Pages repo and pushed (`aed2eee`). The arbitrage.html page was broken because the symlink didn't exist in the deployed repo.

4. **App migration assessment** — `docs/superpowers/plans/2026-03-24-app-migration-plan.md`. Covers 5 apps: arbitrage (import path fix), financial-markets (same), monopoly (no change), information-partition (candidate for migration), backward-induction (defer).

### How to run the implementation

Execute tasks sequentially in this session. After each task:
1. Run `node --test test/plotting-*.test.js` (task-specific tests)
2. Run `node --test` (full suite for regression)
3. Commit if all pass
4. Move to next task

Use the `tikz-svg-builder` skill (invoke with `/tikz-svg-builder`) for any questions about library architecture or PGF source references during implementation.
