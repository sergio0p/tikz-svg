...
export function render(svgEl, config) {
  ...
  for (let i = 0; i < plots.length; i++) {
    ...
    plotModels.push({
      ...
      frame: plotDef.frame ?? null,
      className: plotDef.className ?? null,
    });
  }
  ...
  for (const id of stateIds) {
    ...
    model.nodes[id] = {
      ...
      frame: states[id].frame ?? null,
      className: states[id].className ?? null,
    };
  }
  ...
  for (let i = 0; i < edges.length; i++) {
    ...
    model.edges.push({
      ...
      frame: edges[i].frame ?? null,
      className: edges[i].className ?? null,
    });
  }
...
// renderAutomaton removed — use render() directly (see deprecated/automata-wrapper/)
