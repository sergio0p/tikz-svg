...
/**
 * Check whether frame 1 is included in a Beamer overlay spec string.
 * Specs: '1', '1-', '1-3', '2-', '3', '1,3,5'
 * @param {string} frameSpec
 * @returns {boolean}
 */
function frameIncludesFirst(frameSpec) {
  if (!frameSpec) return true;
  for (const part of frameSpec.split(',')) {
...
/**
 * Extract the maximum frame number from a Beamer overlay spec.
 * Open-ended specs like '2-' return just the start number.
 * @param {string} frameSpec
 * @returns {number}
 */
function maxFrameNumber(frameSpec) {
  if (!frameSpec) return 0;
  let max = 0;
  for (const part of frameSpec.split(',')) {
    const trimmed = part.trim();
    const range = trimmed.match(/^(\d+)\s*-\s*(\d*)$/);
    if (range) {
      max = Math.max(max, parseInt(range[1], 10));
      if (range[2]) max = Math.max(max, parseInt(range[2], 10));
    } else {
...
/**
 * Apply animation metadata (id, data-frame, className, visibility) to an SVG element.
 * @param {SVGElement} el - Target element
 * @param {Object} meta - { id, frame, className }
 * @param {string} [idPrefix] - Optional prefix for id collision prevention
 * @param {Object} refs - The refs object to populate byId
 * @param {Object} frameTracker - { max: number } to track max frame number
 */
function applyAnimationMeta(el, meta, idPrefix, refs, frameTracker) {
  if (!meta) return;
  const { id, frame, className } = meta;

  if (id) {
    const fullId = idPrefix ? `${idPrefix}--${id}` : id;
    el.setAttribute('id', fullId);
    if (refs.byId) refs.byId[fullId] = el;
  }

  if (frame) {
    el.setAttribute('data-frame', frame);
    if (!frameIncludesFirst(frame)) {
      el.setAttribute('visibility', 'hidden');
    }
    const n = maxFrameNumber(frame);
    if (n > frameTracker.max) frameTracker.max = n;
  }
...
function emitPlot(plotModel, layer) {
  ...
  if (path) {
    ...
    primaryEl = createSVGElement('path', attrs);
    layer.appendChild(primaryEl);
  }
...
function emitDrawPath(pathModel, edgeLayer, labelLayer) {
  ...
  const primaryEl = createSVGElement('path', attrs);
  if (pathModel.useAsBoundingBox) primaryEl.setAttribute('data-use-as-bbox', '1');
  edgeLayer.appendChild(primaryEl);
 ...
 * @returns {SVGGElement}
 */
function emitNode(id, node, prng, idPrefix) {
...
export function emitSVG(svgEl, resolved) {
  ...
  // Determine ID prefix for collision prevention
  const idPrefix = svgEl.id || '';

  if (drawOrder) {
    const refs = { nodes: {}, edges: [], labels: [], plots: [], paths: [], byId: {}, frameCount: 0 };
    const frameTracker = { max: 0 };
    ...
    const emitItem = (item, target) => {
      switch (item.type) {
        case 'node': {
          ...
          if (!node) break;
          const g = emitNode(item.id, node, prng, idPrefix);
          // Node auto-ID is set by emitNode; apply frame/className only
          applyAnimationMeta(g, { id: null, frame: node.frame, className: node.className }, idPrefix, refs, frameTracker);
          target.appendChild(g);
          refs.nodes[item.id] = g;
          refs.byId[g.getAttribute('id')] = g;
        ...
        case 'edge': {
          ...
          applyAnimationMeta(pathEl, edge, idPrefix, refs, frameTracker);
          target.appendChild(pathEl);
          refs.edges.push(pathEl);
          const labelEl = emitLabelNode(edge);
          if (labelEl) {
            applyAnimationMeta(labelEl, { id: edge.labelId, frame: edge.frame, className: edge.className }, idPrefix, refs, frameTracker);
        ...
        case 'plot': {
          ...
          if (plotEl) {
            applyAnimationMeta(plotEl, plotModel, idPrefix, refs, frameTracker);
            refs.plots.push(plotEl);
          }
          break;
        }
        case 'drawPath': {
          ...
          if (pathEl) {
            applyAnimationMeta(pathEl, pathModel, idPrefix, refs, frameTracker);
            refs.paths.push(pathEl);
          }
          break;
        }
      }
  ...
  for (const [id, node] of Object.entries(nodes)) {
    const g = emitNode(id, node, prng, idPrefix);
    applyAnimationMeta(g, { id: null, frame: node.frame, className: node.className }, idPrefix, refs, frameTracker);
