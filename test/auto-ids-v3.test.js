/**
 * Auto-ID assignment for edges, edge labels, plots, and paths in src-v3.
 *
 * Rules:
 *  - Nodes already receive ids from their config key (existing behavior).
 *  - Edges: `edge-<from>-<to>` when unique; `edge-<from>-<to>-<n>` (1-indexed)
 *    when multiple edges share the same `from`→`to` pair. Bent variants and
 *    self-loops are treated as ordinary edges for this count.
 *  - Edge labels: `label-<from>-<to>` or `label-<from>-<to>-<n>`, mirroring the edge.
 *  - Plots: `plot-<i>` (0-indexed).
 *  - Paths: `path-<i>` (0-indexed).
 *  - User-supplied `id` wins verbatim.
 *  - All auto-IDs flow through `idPrefix` when the target <svg> has an id.
 */
...
const { render } = await import('../src-v3/index.js');
...
describe('src-v3 auto-IDs: edges', () => {
  ...
  it('suffixes duplicate edges with 1-indexed counters', () => {
    ...
    assert.ok(svg.querySelector('path[id="edge-q0-q1-1"]'), 'first duplicate has suffix 1');
    assert.ok(svg.querySelector('path[id="edge-q0-q1-2"]'), 'second duplicate has suffix 2');
    assert.strictEqual(svg.querySelector('path[id="edge-q0-q1"]'), null, 'no unsuffixed id when duplicated');
  });

  it('bent variant counts as multi-edge with an otherwise-straight edge', () => {
    const svg = makeSvg();
    render(svg, {
      states: {
        q0: { initial: true },
...
describe('src-v3 auto-IDs: edge labels', () => {
...
describe('src-v3 auto-IDs: plots', () => {
...
describe('src-v3 auto-IDs: paths', () => {
...
describe('src-v3 auto-IDs: idPrefix composition', () => {
  it('prefixes all auto-IDs with the svg element id', () => {
    const svg = makeSvg('fig1');
    render(svg, {
      states: {
        q0: { initial: true },
        q1: { position: { right: 'q0' } },
    ...
    assert.ok(svg.querySelector('path[id="fig1--edge-q0-q1"]'), 'edge has prefix');
    assert.ok(svg.querySelector('g.label-node[id="fig1--label-q0-q1"]'), 'label has prefix');
    assert.ok(svg.querySelector('[id="fig1--path-0"]'), 'path has prefix');
  });
});

describe('src-v3 auto-IDs: refs.byId', () => {
  it('populates byId with every auto-ID', () => {
    const svg = makeSvg();
    ...
    assert.ok(refs.byId['node-q0'], 'node-q0 in byId');
    assert.ok(refs.byId['node-q1'], 'node-q1 in byId');
    assert.ok(refs.byId['edge-q0-q1-1'], 'edge-q0-q1-1 in byId');
    assert.ok(refs.byId['edge-q0-q1-2'], 'edge-q0-q1-2 in byId');
    assert.ok(refs.byId['label-q0-q1-1'], 'label-q0-q1-1 in byId');
    assert.ok(refs.byId['label-q0-q1-2'], 'label-q0-q1-2 in byId');
    assert.ok(refs.byId['plot-0'], 'plot-0 in byId');
    assert.ok(refs.byId['path-0'], 'path-0 in byId');
  });
});
